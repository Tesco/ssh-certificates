...
@Tag("UnitTest")
public class EcdsaSshCertificateEncodingAndSigningTest extends AbstractCertificateEncodingAndSigningTest {
    ...
    public void showDecodeAndEncodeToOriginalValue(String curve) {
        // given - a CA key pair
        exec("ssh-keygen", "-t", "ecdsa", "-b", curve.replace("nistp", ""), "-f", caPrivateKey.getAbsolutePath(), "-q", "-N", "");
        // and - a leaf key pair
        exec("ssh-keygen", "-t", "ecdsa", "-b", curve.replace("nistp", ""), "-f", leafPrivateKey.getAbsolutePath(), "-q", "-N", "");
        ...
        SshCertificate<ECPublicKey> decodedCertificate = sshCertificateAlgorithm.decoder().decode(leafCertificateFileContents);
    ...
    public void showEncodeAndSignECSshCertificateForUser(String curve) {
        // given - a CA key pair
        ...
        // and
        EcdsaSshCertificate decodedCertificate = (EcdsaSshCertificate) sshCertificateAlgorithm.decoder().decode(leafCertificate);
        EcdsaSshCertificate ecdsaCertificate =
                (EcdsaSshCertificate) new EcdsaSshCertificate(sshCertificateAlgorithm)
                        .nonce(decodedCertificate.nonce())
                        .curve(curve.toLowerCase())
                        .publicKey((ECPublicKey) leafKeyPair.getPublic())
                        ...
                        .reserved("");
        SshCertificateSigner.sign(ecdsaCertificate, signingKeyPair);
        // ECDSA signatures contain a random nonce so it is not possible to reproduce the identical signature bytes
        ecdsaCertificate.signature(decodedCertificate.signature());

        // when
        String encodeCertificate = sshCertificateAlgorithm.encoder().encodeToString(ecdsaCertificate, "user@host");
    ...
    public void showEncodeAndSignECSshCertificateForHost(String curve) {
        // given - a CA key pair
        exec("ssh-keygen", "-t", "ecdsa", "-b", curve.replace("nistp", ""), "-f", caPrivateKey.getAbsolutePath(), "-q", "-N", "", "-m", "PKCS8");
        KeyPair signingKeyPair = getKeyPair("EC", caPublicKey, caPrivateKey);
        // and - a leaf key pair
    ...
    public void showEncodeECSshCertificateForUserParsableBySshKeyGen(String curve) {
        ...
        EcdsaSshCertificate ecdsaCertificate =
                (EcdsaSshCertificate) new EcdsaSshCertificate(sshCertificateAlgorithm)
                        ...
                        .reserved("");
        SshCertificateSigner.sign(ecdsaCertificate, signingKeyPair);
        sshCertificateAlgorithm.encoder().encodeToFile(ecdsaCertificate, "user@host", leafCertificate);
        ...
        assertThat(parsedValue.get("PublicKey"), equalTo("ECDSA-CERT SHA256:" + BASE64_ENCODER_NO_PADDING.encodeToString(DigestUtils.sha256(sshCertificateAlgorithm.encoder().encodeSignatureKey(ecdsaCertificate.publicKey())))));
        assertThat(parsedValue.get("SigningKey"), containsString("ECDSA SHA256:" + BASE64_ENCODER_NO_PADDING.encodeToString(DigestUtils.sha256(sshCertificateAlgorithm.encoder().encodeSignatureKey(ecdsaCertificate.signatureKey())))));
    ...
    public void showEncodeECSshCertificateForUserWithRSASignatureParsableBySshKeyGen(String curve, int keyLength) {
        ...
        KeyPair signingKeyPair = rsaKeyPairGenerator.generateKeyPair();
        // and - a leaf key pair
        ...
        KeyPair leafKeyPair = ecKeyPairGenerator.generateKeyPair();
        // and - some certificate properties
        SshCertificateAlgorithm<ECPublicKey> sshCertificateAlgorithm = (SshCertificateAlgorithm<ECPublicKey>) SshCertificateAlgorithm.get("ecdsa-sha2-" + curve + "-cert-v01@openssh.com");
        String serialNumber = new BigInteger(63, new Random()).toString();
        String keyId = UUID.randomUUID().toString();
        ...
        assertThat(parsedValue.get("Type"), equalTo(sshCertificateAlgorithm.algorithmName() + " user certificate"));
        assertThat(parsedValue.get("PublicKey"), equalTo("ECDSA-CERT SHA256:" + BASE64_ENCODER_NO_PADDING.encodeToString(DigestUtils.sha256(sshCertificateAlgorithm.encoder().encodeSignatureKey(ecdsaCertificate.publicKey())))));
        assertThat(parsedValue.get("SigningKey"), containsString("RSA SHA256:" + BASE64_ENCODER_NO_PADDING.encodeToString(DigestUtils.sha256(sshCertificateAlgorithm.encoder().encodeSignatureKey(ecdsaCertificate.signatureKey())))));
        assertThat(parsedValue.get("KeyID"), equalTo("\"" + keyId + "\""));
        assertThat(parsedValue.get("Serial"), equalTo(serialNumber));
        assertThat(parsedValue.get("Valid"), equalTo("from 2010-01-01T12:30:00 to 2011-01-01T12:30:00"));
        assertThat(parsedValue.get("Principals"), equalTo("principle_one principle_two"));
    ...
    public void showEncodeECSshCertificateForHostParsableBySshKeyGen(String curve) {
        // given - an EC key generator
        KeyPairGenerator keyPairGenerator = KeyPairGenerator.getInstance("EC");
        keyPairGenerator.initialize(new ECGenParameterSpec(SUPPORTED_EC_CURVES.get(curve)));
        // and - a CA key pair
